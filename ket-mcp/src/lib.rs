        ...
        ToolDescriptor {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    ...
                    "edge_kind": { "type": "string", "enum": ["grounds", "derives", "proposes", "confirms", "refutes", "supersedes"], "description": "Epistemic edge kind for parent links. grounds = irreducible input (axiom, measurement); derives = logically follows (default); proposes = suggested but not entailed (hypothesis); confirms / refutes = verification evidence for / against the parent; supersedes = this node replaces the parent as canonical, parent stays addressable" },
